...
thumb1_rtx_costs (rtx x, enum rtx_code code, enum rtx_code outer)
{
  ...
  switch (code)
    ...
    case MULT:
      if (mode == DImode)
        {
	  /* Always a call optimized ARM procedure */
	  return COSTS_N_INSNS (1);
        }

      if (mode == SImode)
        {
	  /* MUL instruction takes 3 cycles on ARMv5TE and higher */
...
arm_loaded_global_var (rtx insn, rtx * offset_reg, rtx * offset_insn)
{
  ...
  if (set && MEM_P (SET_SRC (set))
      && (GET_CODE (XEXP (SET_SRC (set),0)) == PLUS))
    {
      ...
      if ((op0 != pic_reg) || (!REG_P (op1)))
        return NULL_RTX;

      regno = REGNO (op1);
      if ((DF_REG_USE_COUNT (regno) != 1) || (DF_REG_DEF_COUNT (regno) != 1))
        return NULL_RTX;
                         ...
                         rtx address_reg, rtx load_insn, rtx offset_insn)
{
  ...
  /* The first insn:
         (SET offset_reg (address_of_GOT_entry(symbol) - pc))
